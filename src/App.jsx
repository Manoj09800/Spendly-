...
function Logo({ size = 28 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
      <circle cx="60" cy="60" r="58" fill={COLORS.void} stroke={COLORS.jade} strokeWidth="2" />
      <circle cx="60" cy="60" r="46" fill={COLORS.jade} />
      <text x="60" y="78" fontFamily="Georgia, 'Times New Roman', serif" fontSize="52" fontWeight="700" fill={COLORS.void} textAnchor="middle">
        S
      </text>
      <path d="M28 82 L44 66 L57 75 L92 38" stroke={COLORS.gold} strokeWidth="5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="92" cy="38" r="5" fill={COLORS.gold} />
    </svg>
  );
}
...
function MoneyManager({ session, themeMode, onToggleTheme }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!acctRes.error && acctRes.data) {
        setAccounts(acctRes.data.map((a) => ({ id: a.id, name: a.name })));
      }
      setLoading(false);
    })();
  }, [user.id]);
  ...
  async function addAccount(name) {
    ...
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) { showToast("Couldn't add account"); return null; }
  ...
  async function deleteAccount(id) {
    ...
    if (error) { showToast("Couldn't remove account"); return; }
  ...
  async function addManyTxns(list) {
    ...
    if (error) { showToast("Couldn't save — try again"); return; }
    setTxns((prev) => [
      ...data.map((r) => ({ id: r.id, type: r.type, amount: Number(r.amount), category: r.category, note: r.note || "", date: r.date, account: r.account || "" })),
      ...prev,
    ]);
    setTab("home");
  ...
  return (
    <Shell>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
        ...
        {tab === "reports" && <ReportsView txns={sorted} onDelete={deleteTxn} accounts={accounts} />}
        {tab === "statement" && <StatementView onBulkAdd={addManyTxns} accounts={accounts} onAddAccount={addAccount} />}
      </div>
...
function ReportsView({ txns, onDelete, accounts }) {
  ...
  const byAccount = useMemo(() => {
    ...
    for (const t of filteredTxns) {
      if (t.type !== "expense") continue;
      const key = t.account || "Unassigned";
      ...
      {txns.length === 0 ? (
        ...
        <>
          ...
          {byAccount.length > 0 && (
            <div className="rounded-2xl p-4 mb-5" style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}` }}>
              <div className="f-body text-sm mb-3" style={{ color: COLORS.muted }}>
                By account {selectedMonth !== "all" ? `— ${selectedMonth}` : ""}
              </div>
              <div className="flex flex-col gap-2">
                {byAccount.map((a) => {
                  const max = byAccount[0].value || 1;
                  const pct = Math.max(6, (a.value / max) * 100);
                  return (
                    <div key={a.name}>
                      <div className="f-body flex items-center justify-between text-sm mb-1">
                        <span>{a.name}</span>
                        <span className="f-mono">₹{fmt(a.value)}</span>
                      </div>
                      <div className="w-full h-1.5 rounded-full" style={{ background: COLORS.panelRaised }}>
                        <div className="h-1.5 rounded-full" style={{ width: `${pct}%`, background: COLORS.jade }} />
                      </div>
                    </div>
                  );
                })}
          ...
          {dailyBreakdown && dailyBreakdown.length > 0 && (
            <div className="rounded-2xl p-4 mb-5" style={{ background: COLORS.panel, border: `1px solid ${COLORS.line}` }}>
              <div className="f-body text-sm mb-2" style={{ color: COLORS.muted }}>
                Daily spend — {selectedMonth}
              </div>
              <div style={{ width: "100%", height: 160 }}>
                <ResponsiveContainer>
                  <BarChart data={dailyBreakdown}>
                    <CartesianGrid strokeDasharray="3 3" stroke={COLORS.line} vertical={false} />
                    <XAxis dataKey="day" stroke={COLORS.muted} fontSize={10} tickLine={false} axisLine={false} interval={2} />
                    <YAxis stroke={COLORS.muted} fontSize={11} tickLine={false} axisLine={false} width={30} />
                    <Tooltip
                      contentStyle={{ background: COLORS.panelRaised, border: `1px solid ${COLORS.line}`, borderRadius: 8, fontSize: 12 }}
                      formatter={(v) => [`₹${fmt(v)}`, "Spent"]}
                      labelFormatter={(d) => `${d} ${selectedMonth}`}
                    />
                    <Bar dataKey="value" fill={COLORS.coral} radius={[3, 3, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
...
function AddSheet({ onClose, onSave, accounts, onAddAccount, onDeleteAccount }) {
  ...
  async function confirmNewAccount() {
    const name = await onAddAccount(newAccountName);
    if (name) setAccount(name);
    setNewAccountName("");
  ...
  function submit() {
    ...
    if (!n || n <= 0) return;
    onSave({ type, amount: n, category, note: note.trim(), date: new Date(date).toISOString(), account });
  }
    ...
    <div className="absolute inset-0 z-20 flex items-end" style={{ background: "#000000aa" }} onClick={onClose}>
      ...
      >
        ...
        <input
          ...
          className="f-mono w-full rounded-xl px-3 py-2.5 mb-4 outline-none text-sm"
          style={{ background: COLORS.panelRaised, border: `1px solid ${COLORS.line}`, color: COLORS.text }}
        />

        <label className="text-xs mb-2 block" style={{ color: COLORS.muted }}>Account (optional)</label>
        <div className="flex flex-wrap gap-2 mb-6">
          <button
            onClick={() => setAccount("")}
            className="px-3 py-1.5 rounded-full text-xs"
            style={{
              background: account === "" ? COLORS.jade : "transparent",
              border: `1px solid ${account === "" ? COLORS.jade : COLORS.line}`,
              color: account === "" ? COLORS.void : COLORS.muted,
            }}
          >
            None
          </button>
          {accounts.map((a) => (
            <button
              ...
              className="px-3 py-1.5 rounded-full text-xs"
              style={{
                background: account === a.name ? COLORS.jade : "transparent",
                border: `1px solid ${account === a.name ? COLORS.jade : COLORS.line}`,
                color: account === a.name ? COLORS.void : COLORS.muted,
              }}
            >
              {a.name}
            </button>
          ...
          {!addingAccount && (
            <button
              ...
              className="px-3 py-1.5 rounded-full text-xs"
              style={{ border: `1px dashed ${COLORS.jade}`, color: COLORS.jade }}
            >
              + Add account
            </button>
          )}
...
function StatementView({ onBulkAdd, accounts, onAddAccount }) {
  ...
  function save() {
    const toAdd = rows
      ...
      .map((r) => ({ type: r.type, amount: r.amount, category: r.category, note: r.description, date: r.date, account: batchAccount }));
    ...
    <div className="px-5 pt-8">
      ...
      {rows.length > 0 && (
        <>
          ...
          <div className="f-body text-xs mb-2" style={{ color: COLORS.muted }}>
            Tag this whole import with an account (optional)
          </div>
          <div className="flex flex-wrap gap-2 mb-5">
            <button
              onClick={() => setBatchAccount("")}
              className="px-3 py-1.5 rounded-full text-xs"
              style={{
                background: batchAccount === "" ? COLORS.jade : "transparent",
                border: `1px solid ${batchAccount === "" ? COLORS.jade : COLORS.line}`,
                color: batchAccount === "" ? COLORS.void : COLORS.muted,
              }}
            >
              None
            </button>
            {accounts.map((a) => (
              <button
                key={a.id}
                onClick={() => setBatchAccount(a.name)}
                className="px-3 py-1.5 rounded-full text-xs"
                style={{
                  background: batchAccount === a.name ? COLORS.jade : "transparent",
                  border: `1px solid ${batchAccount === a.name ? COLORS.jade : COLORS.line}`,
                  color: batchAccount === a.name ? COLORS.void : COLORS.muted,
                }}
              >
                {a.name}
              </button>
